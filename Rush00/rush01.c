...
/*   Updated: 2024/11/09 16:05:46 by jnavalla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <unistd.h>
...
First we check if it is a line or a point instead of an square
because they are special solutions (any x = 1 or y = 1) with 
first 3 ifs (if it is a "square")
4º if = if we are in (1,1) or (x,y) then returns "/"
5º if = if we are in (x,1) or (1,y) then returns "\"
6º if = any position on any edge is "*" (/ or \ are already done in 4º and 5º)
...
char	ft_what_character(int x, int y, int num_row, int num_column)
{
	...
	if (x == 1 && num_row == y)
		return ('\\');
	if ((num_row == 1 && num_column == 1) || (num_row == y && num_column == x))
	{
		return ('/');
	}
	if ((num_row == 1 && num_column == x) || (num_row == y && num_column == 1))
	{
		return ('\\');
	}
