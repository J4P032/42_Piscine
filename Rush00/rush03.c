...
/*   Updated: 2024/11/09 16:29:02 by jnavalla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <unistd.h>
...
1º if = if we are in (1,1) or (1,y) then returns "A"
2º if = if we are in (x,1) or (x,y) then returns "C"
...
{
	if ((num_row == 1 && num_column == 1) || (num_row == y && num_column == 1))
	{
		return ('A');
	}
	if ((num_row == 1 && num_column == x) || (num_row == y && num_column == x))
	{
		return ('C');
	}
