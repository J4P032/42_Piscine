...
/*   Updated: 2024/11/09 16:37:40 by jnavalla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include <unistd.h>
...
first 3 ifs 
(if it is a "square")
4º if = if we are in (1,1) or (x,y) then returns "A"
5º if = if we are in (x,1) or (1,y) then returns "C"
6º if = any position on any edge is "B" (A or C are already done in 4º and 5º)
